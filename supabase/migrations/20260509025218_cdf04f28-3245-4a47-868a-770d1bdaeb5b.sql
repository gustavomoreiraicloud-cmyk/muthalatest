...
BEGIN
  ...
  -- Validate each item against menu_items and recompute subtotal
  ...
  LOOP
    ...
    -- Look up authoritative base price (case-insensitive by name)
    ...
    -- Client-supplied price must be at least the base menu price.
    -- Options/extras only ever add to the price, so price < base = tampering.
