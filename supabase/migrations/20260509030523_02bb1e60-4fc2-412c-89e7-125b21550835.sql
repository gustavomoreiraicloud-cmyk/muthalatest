...
DROP FUNCTION IF EXISTS public.auto_grant_admin_for_username() CASCADE;
...
DECLARE
  ...
  item_qty integer;
  item_name text;
  menu_base_price numeric;
  validated_delivery_fee numeric := 0;
  max_allowed_fee numeric;
BEGIN
  IF _subtotal IS NULL OR _subtotal < 0 THEN RAISE EXCEPTION 'Subtotal inválido'; END IF;
  IF _delivery_fee IS NULL OR _delivery_fee < 0 THEN RAISE EXCEPTION 'Taxa de entrega inválida'; END IF;
  IF _total IS NULL OR _total < 0 THEN RAISE EXCEPTION 'Total inválido'; END IF;
  IF _discount IS NULL OR _discount < 0 THEN RAISE EXCEPTION 'Desconto inválido'; END IF;
  ...
  IF _points_used > 0 THEN
    IF _user_id IS NULL THEN
      RAISE EXCEPTION 'Pontos requerem usuário autenticado';
    END IF;
    IF auth.uid() IS NULL OR auth.uid() <> _user_id THEN
      RAISE EXCEPTION 'Não autorizado a usar pontos de outro usuário';
    END IF;
  END IF;
  ...
  LOOP
    ...
    IF item_price < 0 OR item_qty <= 0 OR length(trim(item_name)) = 0 THEN
      RAISE EXCEPTION 'Item inválido no pedido';
    END IF;

    SELECT price INTO menu_base_price
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Item não disponível: %', item_name;
    END IF;

    IF item_price + 0.05 < menu_base_price THEN
      RAISE EXCEPTION 'Preço inválido para o item: %', item_name;
    END IF;
  ...
  IF _delivery_method = 'retirada' OR _delivery_method = 'pickup' THEN
    validated_delivery_fee := 0;
  ELSE
    SELECT max(fee) INTO max_allowed_fee
      FROM public.delivery_ranges
     WHERE COALESCE(active, true) = true;

    IF max_allowed_fee IS NULL THEN
      SELECT delivery_fee INTO max_allowed_fee
        FROM public.store_settings
       ORDER BY updated_at DESC NULLS LAST
       LIMIT 1;
      max_allowed_fee := COALESCE(max_allowed_fee, 0);
    END IF;

    IF EXISTS (
      SELECT 1 FROM public.delivery_ranges
       WHERE COALESCE(active, true) = true
         AND abs(fee - _delivery_fee) <= 0.05
    ) THEN
      validated_delivery_fee := _delivery_fee;
    ELSIF _delivery_fee <= max_allowed_fee + 0.05 AND _delivery_fee >= 0 THEN
      IF _delivery_fee < 0.01 AND NOT EXISTS (
        SELECT 1 FROM public.delivery_ranges
         WHERE COALESCE(active, true) = true AND fee < 0.01
      ) THEN
  ...
  IF _coupon_code IS NOT NULL AND length(trim(_coupon_code)) > 0 THEN
    ...
    END IF;
  ELSE
    IF _discount > 0 THEN RAISE EXCEPTION 'Desconto sem cupom'; END IF;
    validated_discount := 0;
  END IF;

  expected_total := computed_subtotal - validated_discount + validated_delivery_fee;
  ...
  INSERT INTO public.orders (
    ...
    validated_delivery_fee, expected_total, _payment_method, _address_street, _address_number,
